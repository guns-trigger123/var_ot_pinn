...
class pyOMT_raw():
    ...
    def __init__ (self, h_P, num_P, dim, max_iter, lr, bat_size_P, bat_size_n, nu_mass_P= None):
        ...
        if self.nu_mass_P is not None:
            self.d_nu_mass_P = self.nu_mass_P.cuda()
        ...
        self.zero_location = torch.zeros(self.num_P, dtype=torch.bool)
    ...
    def gen_pre_cal(self,count):
        self.d_volP.copy_(torch.rand_like(self.d_volP,dtype=torch.float,device=torch.device('cuda')))
        self.d_volP.add_(-0.5)
    ...
    def cal_measure(self, cal_c=False):
        ...
        '''
        self.d_tot_ind_val.fill_(-1e30)  # a very low value
        self.d_tot_ind.fill_(-1)  # index equaling -1 means not assigned yet
        i = 0
        while i < self.num_P // self.bat_size_P:
            temp_P = self.h_P[i*self.bat_size_P:(i+1)*self.bat_size_P]
            temp_P = temp_P.view(temp_P.shape[0], -1) # useless?

            '''U=PX+H'''
            self.d_temp_h = self.d_h[i*self.bat_size_P:(i+1)*self.bat_size_P]
            self.d_temp_P.copy_(temp_P)
            torch.mm(self.d_temp_P, self.d_volP.t(),out=self.d_U)  # d_U shape: batch_size_P * batch_size_n
        ...
        if cal_c == True:
            self.zero_location.copy_(torch.bincount(self.d_tot_ind, minlength=self.num_P) == 0)
    ...
    def update_h(self):
        ...
        if self.nu_mass_P is None:
            self.d_g -= 1./self.num_P  # nu_i = 1./self.num_P, and to be used to detect h_i without MC samples
        else:
            self.d_g -= self.d_nu_mass_P
        self.d_adam_m *= 0.9
    ...
    def run_gd(self, last_step=0, num_bat=1):
        ...
        while(steps <= self.max_iter):
            ...
            if self.nu_mass_P is None:
                num_zero = torch.sum(self.d_g == -1./self.num_P) # detect h_i without MC samples
            else:
                num_zero = torch.sum(self.d_g == -self.d_nu_mass_P)
